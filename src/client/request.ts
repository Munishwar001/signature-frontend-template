...
export class RequestClient extends Client {
	...
	async fetchRequestData(id: any) {
        try {
			...
			// alert("sending the fetch request");
   ...
   async deleteWholeTemplate(id:String){
	try{
		 const res = await this.request("DELETE",  `/api/templates/deleteWholeTemplate/${id}`);
